...
func (op *httpsGetNodesInfoOp) shouldUseResponse(host string, nodesStates *nodesStateInfo) bool {
	...
	for _, node := range nodesStates.NodeList {
		if node.Address == host {
			...
			// if the above is true then we're looking at a node in the main/sandbox that the endpoint was called on
			// since k8s may pass a host that's a service name rather than a specific node we need a different way of
			// figuring out which main/sandbox cluster the results were retrieved from
